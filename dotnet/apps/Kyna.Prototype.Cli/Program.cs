...
DatabaseLogService? dbLogService;
...
try
{
    ...
    else
    {
        ...
        Debug.Assert(dbLogService is not null);

        // Clear logs and app_events tables.
        await dbLogService.RemoveLogsAsync();
        await dbLogService.RemoveAppEventsAsync();

        Communicate($"verbose  = {config.Verbose}", true);
        Communicate($"showHelp = {config.ShowHelp}", true);

        Communicate("Testing communication logging", false, LogLevel.Debug, defaultScope);

        // Log app started event.
        KLogger.LogEvent(EventIdRepository.GetAppStartedEvent(config), processId);

        // test logger scope.
        using (logger.BeginScope("prototype-scope"))
        {
            logger.LogInformation("This is a message within scope.");
        }

        // test logger without scope.
        logger.LogInformation("This is a message without scope.");
        ...
        {
            KLogger.LogCritical(exc, processId: processId);
        }
    }
    exitCode = 0;
}
